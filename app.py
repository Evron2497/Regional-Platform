...
st.markdown("""
     ...
     /* Hides the top right colored decoration badge */
     [data-testid="stDecoration"] {
         display: none !important;
     }
     ...
     /* --- FLOATING LEFT BOTTOM SWIPE INDICATOR --- */
     .mobile-sidebar-hint {
         position: fixed;
         bottom: 20px;
         left: 20px;
...
# --- SIDEBAR CONTENT PANEL ---
with st.sidebar:
     # --- ENHANCED FEATURE: RESUME SESSION DETERMINED BY TRANSACTION TYPE ---
     st.header("🔑 Resume Session")
     ...
     if st.button("🚀 Restore My Chat Session", key="sidebar_recovery_btn"):
         if recovery_tx_id:
             all_profiles = db.get_profiles()
             found_profile_id = None
             transaction_type = "chat" # Default fallback
             
             conn = db.get_db()
             cursor = conn.cursor()
             try:
                 cursor.execute("SELECT profile_id, type FROM manual_transactions WHERE transaction_id = ?", (recovery_tx_id,))
                 row = cursor.fetchone()
                 if row:
                     found_profile_id = row[0]
                     transaction_type = row[1] # "chat" or "meetup"
             except Exception:
                 pass
             finally:
                 conn.close()
             ...
             if found_profile_id:
                 ...
                 if matched_profile:
                     ...
                     # Intelligently track state based on type of transaction recovered
                     if transaction_type == "meetup":
                         st.session_state[f"entered_tx_chat_{found_profile_id}"] = recovery_tx_id
                         st.session_state[f"entered_tx_meet_{found_profile_id}"] = recovery_tx_id
                         st.success(f"Session Restored successfully for physical meetup routing with: {matched_profile['name']}!")
                     else:
                         st.session_state[f"entered_tx_chat_{found_profile_id}"] = recovery_tx_id
                         st.success(f"Session Restored successfully for chat room: {matched_profile['name']}!")
                     st.rerun()
                 else:
                     st.error("The profile associated with this verification code is no longer active.")
             else:
     ...
     if not st.session_state.admin_logged_in:
         ...
         if not pending_list:
             ...
             for item in pending_list:
                 ...
                 👤 **Target Client:** {item['profile_name'] if item['profile_name'] else 'New Submission'}<br>
                 💵 **Code Claimed:** `{item['transaction_id']}`<br>
                 #️⃣ **Amount Paid:** KES {item['amount']:.2f}
                 """, unsafe_allow_html=True)
...
# --- MAIN APP LOGIC ---
if "selected" not in st.session_state:
     # --- MODIFIED SYSTEM DIRECTORY LOGIC ---
     # Fetching all existing base profiles from database repository so they persist indefinitely 
     # until the Admin triggers manual delete action inside management terminal
     profiles = db.get_profiles() 
    
     if not profiles:
         st.info("✨ No profiles currently listed. Please check back shortly!")
     else:
         cols = st.columns(3) 
         for idx, p in enumerate(profiles):
             with cols[idx % 3]:
