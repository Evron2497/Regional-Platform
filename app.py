...
# Establish initial session variables
if "sidebar_state" not in st.session_state:
    st.session_state.sidebar_state = "expanded"  # Defaulting to open for better UX
...
# Background application routing configs
FASTAPI_BACKEND_URL = "http://127.0.0.1:8000" 
...
# ==========================================
# 4. CUSTOM PROFESSIONAL UI STYLING (CSS)
# ==========================================
st.markdown("""
    <style>
    /* Clean stream layouts removing default wrappers */
    ...
    .chevron-icon { font-family: monospace; font-weight: 900; letter-spacing: -2px; font-size: 16px; opacity: 0.9; }
    @keyframes pulseLeftHint { 0% { transform: scale(1); } 50% { transform: scale(1.04); } 100% { transform: scale(1); } }
    </style> 
""", unsafe_allow_html=True)

# ==========================================
# 5. CONTROL HEADER AND STICKY MENU BAR
# ==========================================
...
# Apply the sidebar toggle state natively
st.markdown(f'<div data-sidebar-state="{st.session_state.sidebar_state}"></div>', unsafe_allow_html=True)
...
# ==========================================
# Render sidebar elements conditionally based on the state setting
if st.session_state.sidebar_state == "expanded":
    with st.sidebar:
         st.header("🔑 Resume Session")
         st.markdown("Enter your paid Transaction ID to recover your corresponding setup environment (Chat vs Meetup):")
