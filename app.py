...
st.markdown("""
    ...
    .welcome-banner { text-align: center; background-color: #64F58B; padding: 15px; border-radius: 10px; margin-bottom: 20px; color: black; }
    .mobile-sidebar-hint {
...
# Mobile sidebar shortcut tooltip
st.markdown("""
     ...
     with st.expander("📝 Fill Submission Form (Client Side Only)", expanded=False):
         ...
         if st.button("🔓 Submit Profile for Verification", key="sub_verify_btn"):
             if not sub_tx_id or not sub_name:
                 ...
                 # Store serialized profile data inside the account reference parameter
